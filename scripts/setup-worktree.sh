...
echo "[3/6] generate firebase.worktree.json..."
...
# ----- .worktree.env 生成 -----
echo "[4/6] generate .worktree.env..."
...
# ----- worktree-init (gitignored ローカルファイルの main → worktree コピー) -----
# pnpm install の前に走らせる: postinstall フック等が config を読む可能性に備える
echo "[5/6] copy gitignored local files (config / templates / .jarvis)..."
bash "$SCRIPT_DIR/worktree-init.sh" "$WORKTREE_PATH"
echo ""

# ----- pnpm install -----
if [[ "$SKIP_INSTALL" == "true" ]]; then
  echo "[6/6] pnpm install ... [skipped (--skip-install)]"
  echo "  → 後で worktree 内で手動実行してください:"
  echo "      cd $WORKTREE_PATH && pnpm install"
  echo "    (setup-worktree.sh の再実行は worktree 既存エラーで失敗します)"
else
  echo "[6/6] pnpm install (3-5 minutes)..."
