...
apps/frontend/supabase/migrations/
  ...
  20260622060000_titles_fix.sql       ← 新 (修正分を ALTER/DELETE 等で表現)
```
...
- staging Supabase (`womzkyizshrfipvsaals`) の `supabase_migrations.schema_migrations` に「local migration files に存在しない version」が残存している状態
- production への適用は **絶対に行わない**。production の repair は別 SoP (未策定、別タスク化推奨)
...
```sql
-- 例 (PR #249 のケース): titles テーブル + 関連 RLS policy + 8 行 seed を一掃
DROP TABLE public.titles CASCADE;

-- 他にも CREATE TYPE / CREATE INDEX 等があれば追加で DROP
-- ALTER TABLE で追加した CHECK 制約は別途 ALTER ... DROP CONSTRAINT で戻す
```

URL: `https://supabase.com/dashboard/project/womzkyizshrfipvsaals/sql` (Project 名「**Revolution Staging**」を必ず確認)
...
- [ ] staging project ref `womzkyizshrfipvsaals` であることを URL で再確認 (production `abqsntbvnuttpyixagob` と混同しないこと)
- [ ] Phase 1 で current state を記録 (post-mortem 用)
- [ ] Phase 2 の DROP TABLE / DROP CONSTRAINT が対象テーブルのみで CASCADE 範囲を理解
