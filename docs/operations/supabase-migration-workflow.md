...
**一般化ルール (claude[bot] PR #250 review #4 採用)**: 対象 migration の内容に応じて逆操作を実行する。CASCADE の影響範囲は事前に確認 (例: `\d+ <table>` で依存 object を一覧、または `pg_depend` で関連 object を SELECT)。

| migration 操作 | 逆操作 |
|---|---|
| `CREATE TABLE` | `DROP TABLE ... CASCADE` (依存関係を事前に `\d+` で確認) |
| `CREATE INDEX` | `DROP INDEX IF EXISTS` |
| `CREATE TYPE` (ENUM) | `DROP TYPE IF EXISTS` (使用箇所を `pg_type` で確認) |
...
- [ ] Phase 2 の DROP / ALTER 逆操作が対象 object のみで CASCADE 範囲を理解 (§10.2 一般化ルール表を参照)
