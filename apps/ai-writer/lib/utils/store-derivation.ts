 ...
 *
 * 区切りはタイトルが `/`、本文が `・`。タイトルは 28〜40 文字制限があり短い記号が有利で、
...
export function deriveStoreContext(input: DeriveStoreContextInput): StoreContext {
  ...
  const asVenue = (代表店舗名: string): StoreContext => {
    // ★ 作品名が空のときの扱いを Step 5 と揃える (claude[bot] 指摘 2026-08-09)。
    //   従来ここは `代表店舗名` 単体へ落ちており、`## OH MY CAFEのメニュー` のように
    //   **何のイベントか分からない見出し**になりえた。Step 5 側は同じ状況で
    //   見出し主語を空にして呼び出し側へ委ねているため、同じ決定表の中で
    //   分岐ごとに挙動が違う状態だった。
    //   実運用では canonicalWorkTitle が空になることはまず無いが、片方だけ
    //   ガードがある状態は次に読む人を惑わせる。
    if (workTitle.length === 0) {
      warnings.push(
        '作品名が空のため、会場名だけの見出しになるのを避けて見出し主語を空にしました。' +
    ...
    return {
      ...
      都市表記: '',
      見出し主語:
        workTitle.length > 0 && 代表店舗名.length > 0 ? `${workTitle} × ${代表店舗名}` : '',
