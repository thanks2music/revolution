 ...
 * ## 設計
 *
 * - **`order` を必須引数にしている。** `range()` はサーバ側の順序に依存するので、
 *   order を指定しないページングは**ページ間で行が重複・欠落し得る**。
 *   任意引数にすると忘れられるため、型で強制する。
 * - 1 ページの取得数は `db.max_rows` を知らなくても安全な値に固定する。
 *   返却数がページサイズ未満になった時点で終端と判断する。
