 ...
 *
 * ### `data-grid-layout` が本番 DOM に出ることは意図的に受容している
 *
 * テスト専用の属性が `view-source` から見える形になるが、以下の理由で許容する
 * (claude-review 指摘 2026-08-25 を受けて明文化):
 *
 * - **属性を消すと検査できない**。DOM に出さずに意図を宣言する手段が無く、
 *   クラス名から推測する方式に戻すと下記「自作自演」の問題が復活する
 * - 露出するのは `single` / `block` / `row` という**レイアウトの分類名だけ**で、
 *   ビジネスナレッジ (slug の真実値・SEO ノウハウ等) を含まない。SoC の
 *   「public な revolution 側に機密を書かない」規律に抵触しない
 * - バイト数・アクセシビリティ・SEO への実害はない
 *
 * ### 「自作自演」ではない理由
 *
