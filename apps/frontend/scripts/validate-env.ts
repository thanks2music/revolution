...
const envSchema = z.object({
  // 必須環境変数
  NEXT_PUBLIC_WP_ENDPOINT: z.string().url('WordPress GraphQL エンドポイントは有効なURLである必要があります'),
...
function validateEnv(): Env {
  ...
  try {
    ...
    console.log(`  NEXT_PUBLIC_ALLOWED_IMAGE_HOST: ${env.NEXT_PUBLIC_ALLOWED_IMAGE_HOST || '(未設定)'}`);
    ...
    console.log(`  NEXT_PUBLIC_SWR_DEDUPING_INTERVAL: ${env.NEXT_PUBLIC_SWR_DEDUPING_INTERVAL}ms\n`);
...
async function testWordPressConnection(endpoint: string): Promise<void> {
  ...
  try {
    ...
    console.log(`  サイトタイトル: ${data.data?.generalSettings?.title || '(取得できませんでした)'}`);
    ...
    console.warn('⚠️  WordPress が起動していない、またはエンドポイントが正しくない可能性があります\n');
    // 接続失敗は警告のみで、検証は続行
  }
}
